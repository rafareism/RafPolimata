#!/usr/bin/env bash
# build.sh — compile RAFAELIA enterprise para ARM64, x86-64, ARM32
# Estratégia: -O2 deixa compilador usar fmla/crc32cx/lfence nativos
# -nostdlib -ffreestanding: zero libc, zero startup, _start como entrypoint
# -march=native: ARM64 emite crc32cx+neon; x86 emite sse4.2+avx2
set -euo pipefail

echo "=== RAFAELIA Enterprise Build ==="

CFLAGS="-O2 -nostdlib -ffreestanding -fno-stack-protector \
        -fno-asynchronous-unwind-tables -fno-plt \
        -Wall -Wextra -Wno-unused-function \
        -I."

# ── ARM64 (nativo Termux/Android) ────────────────────────────────────────
if [[ "$(uname -m)" == "aarch64" ]]; then
    echo "[ARM64] gcc -march=armv8.2-a+crc+crypto ..."
    gcc $CFLAGS \
        -march=armv8.2-a+crc+crypto \
        -mtune=cortex-a78 \
        -static \
        -e _start \
        -o raf_enterprise_a64 \
        raf_main.c
    echo "[ARM64] OK -> raf_enterprise_a64"
    echo "[ARM64] size:"; size raf_enterprise_a64
fi

# ── x86-64 ───────────────────────────────────────────────────────────────
if [[ "$(uname -m)" == "x86_64" ]]; then
    echo "[x86-64] gcc -march=native ..."
    gcc $CFLAGS \
        -march=native \
        -static \
        -e _start \
        -o raf_enterprise_x64 \
        raf_main.c
    echo "[x86-64] OK -> raf_enterprise_x64"
    size raf_enterprise_x64
fi
# ── ARM32 nativo (Termux em dispositivos 32 bits) ─────────────────────────
if [[ "$(uname -m)" == armv7l || "$(uname -m)" == armv8l || "$(uname -m)" == arm* ]]; then
    echo "[ARM32] compilação nativa com suporte a NEON ..."
    gcc $CFLAGS \
        -march=armv7-a+fp \
        -mfpu=neon-vfpv4 \
        -mfloat-abi=hard \
        -static \
        -e _start \
        -o raf_enterprise_a32 \
        raf_main.c
    echo "[ARM32] OK -> raf_enterprise_a32"
fi
# ── ARM32 cross (se toolchain disponível) ────────────────────────────────
if command -v arm-linux-gnueabihf-gcc &>/dev/null; then
    echo "[ARM32] cross-compile ..."
    arm-linux-gnueabihf-gcc $CFLAGS \
        -march=armv7-a+fp \
        -mfpu=neon-vfpv4 \
        -mfloat-abi=hard \
        -static \
        -e _start \
        -o raf_enterprise_a32 \
        raf_main.c
    echo "[ARM32] OK -> raf_enterprise_a32"
fi

echo "=== Build complete ==="
echo "Binary stripped size target: < 20KB (zero libc linkage)"

# strip para tamanho mínimo
for b in raf_enterprise_*; do
    [[ -f "$b" ]] && strip --strip-all "$b" && echo "stripped: $(du -sh $b)"
done
