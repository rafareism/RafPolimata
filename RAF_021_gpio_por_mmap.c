 ...
 *
 * Mapeia registradores no espaço do processo.
 *
 * Status: skeleton C low-level para Linux/Android/ARM.
 */

static inline uint64_t rafaelia_read_counter_m021(void) {
...
int rafaelia_m021_gpio_por_mmap(void) {
    uint64_t t0 = rafaelia_read_counter_m021();
    uint64_t t1 = rafaelia_read_counter_m021();
    return (t1 >= t0) ? 0 : -1;
}
